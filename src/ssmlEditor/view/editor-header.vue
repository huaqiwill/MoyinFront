...
import { PlayTag } from "@/ssmlEditor/components";
import { exportRaw } from "@/ssmlEditor/utils";
 ...
 */
async function handleCopy(isFormat) {
...
<template>
  <div class="editor-header d-flex flex-row align-item-center justify-content-between">
    ...
    <div
      class="operation-wrapper d-flex flex-row justify-content-center align-items-center"
    >
  ...
  <ElDialog v-model="dialogVisible" title="查看SSML" width="80%">
    ...
    <template #footer>
      <span class="dialog-footer">
        <ElButton type="info" @click="handleCopy(true)">复制+关闭</ElButton>
        <ElButton type="primary" @click="handleCopy(false)"
