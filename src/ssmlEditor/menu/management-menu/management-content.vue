...
const speakerCache = shallowRef([]);
const recentUsageCache = ref([]);
const dataListCategory = ref([{ label: "全部类型", value: "" }, ...tryPlay.category]);
const dataListSpeaker = ref([]);
const dataListRole = ref([]);
const dataListStyle = ref([]);

const dataListSpeed = ref(speed());
const dataListPitch = ref(pitch());
...
function handleSelectSpeaker(name) {
  const speader = speakerCache.value.find((v) => v.name === name);
  function setter(speader) {
...
async function handleRecordRecentUsage(data) {
  try {
...
function handleRecentUsageItemClick(item) {
...
async function handleRecentUsageClose(index) {
  try {
