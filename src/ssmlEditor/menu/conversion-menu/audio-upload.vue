...
import { CancellationTokenSource, FileSelector, Timer } from "@/ssmlEditor/utils";
...
const {
  ...
  fetchSpeaker,
  timeoutMilliseconds,
} = ssmlEditorConfig.conversion;

const boxRef = ref();

const audioInfo = ref();
const transferAudioInfo = ref();
const isRecord = ref(true);
const speakerList = ref([]);
const selSpeaker = ref();

const recordFile = shallowRef();
...
async function handleSpeakerItemClick(item) {
  try {
    if (audioInfo.value) {
      selSpeaker.value = item;
      transferAudioInfo.value = await transfer({
        audioId: audioInfo.value.id,
        speakerId: item.id,
      });
    } else {
...
function handleSubmit() {
  if (transferAudioInfo.value && selSpeaker.value) {
    emit("submit", {
      label: selSpeaker.value.displayName,
      value: transferAudioInfo.value.src,
    });
    reset();
  }
}
